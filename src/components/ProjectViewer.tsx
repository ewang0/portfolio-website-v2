  ...
  return (
    <div className="flex flex-row w-full">
      ...
      <div className="hidden md:flex flex-col w-[60%] overflow-y-auto">
        <div
          className={`flex justify-between sticky top-0 p-5 z-10 ${
        ...
        >
          ...
          <ExternalLink
            href={projectsArr[selected]?.link}
            text="Deployed Project"
          />
        </div>
        <div className="pb-5 px-5">
          <Gallery
            selectedImgUrl={projectsArr[selected]?.image.asset.url}
            skills={projectsArr[selected]?.skills}
            link={projectsArr[selected]?.link}
          />
        </div>
      </div>
    </div>
